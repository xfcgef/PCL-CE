...
#Region "初始化"
    ...
    Private Function GetRequireLoaderData() As CompLocalLoaderData
        ...
        res.CompPath = PageInstanceLeft.Instance.PathIndie & If(PageInstanceLeft.Instance.Info.HasLabyMod, "labymod-neo\fabric\" & PageInstanceLeft.Instance.Info.VanillaName & "\", "") & GetPathNameByCompType(CurrentCompType) & "\"
    ...
    Public Function LoaderRun(Type As LoaderFolderRunType) As Boolean
        ...
        If String.IsNullOrEmpty(CurrentFolderPath) Then
            '加载根目录
            LoadPath = PageInstanceLeft.Instance.PathIndie & If(PageInstanceLeft.Instance.Info.HasLabyMod, "labymod-neo\fabric\" & PageInstanceLeft.Instance.Info.VanillaName & "\", "") & GetPathNameByCompType(CurrentCompType) & "\"
        Else
            '加载当前文件夹
...
#Region "文件夹导航"
    ...
    Private Sub GoBackToParentFolder()
        ...
        Try
            '获取根路径
            Dim rootPath = PageInstanceLeft.Instance.PathIndie & If(PageInstanceLeft.Instance.Info.HasLabyMod, "labymod-neo\fabric\" & PageInstanceLeft.Instance.Info.VanillaName & "\", "") & GetPathNameByCompType(CurrentCompType) & "\"
...
#Region "管理"
    ...
    Private Sub BtnManageOpen_Click(sender As Object, e As EventArgs) Handles BtnManageOpen.Click, BtnHintOpen.Click
        Try
            ...
            If String.IsNullOrEmpty(CurrentFolderPath) Then
                ' 打开根目录
                CompFilePath = PageInstanceLeft.Instance.PathIndie & If(PageInstanceLeft.Instance.Info.HasLabyMod, "labymod-neo\fabric\" & PageInstanceLeft.Instance.Info.VanillaName & "\", "") & GetPathNameByCompType(CurrentCompType) & "\"
            Else
                ' 打开当前子文件夹
    ...
    Public Shared Function InstallMods(FilePathList As IEnumerable(Of String)) As Boolean
        ...
        Else
            '处于 Mod 管理页面
Install:
            Dim ModFolder = targetInstance.PathIndie & If(targetInstance.Info.HasLabyMod, "labymod-neo\fabric\" & targetInstance.Info.VanillaName & "\", "") & "mods\"
            Try
        ...
        Select Case CompType
            Case CompType.Mod
                ...
                If String.IsNullOrEmpty(TargetFolderPath) Then
                    CompFolder = targetInstance.PathIndie & If(targetInstance.Info.HasLabyMod, "labymod-neo\fabric\" & targetInstance.Info.VanillaName & "\", "") & "mods\"
                Else
    ...
    Private Function CanPassFilter(CheckingMod As LocalCompFile) As Boolean
        Select Case Filter
            ...
            Case FilterType.Duplicate
                ...
                Return ItemSource IsNot Nothing AndAlso ItemSource.Where(Function(m) CheckingMod.Comp IsNot Nothing AndAlso m.Comp IsNot Nothing AndAlso CheckingMod.Comp.Id = m.Comp.Id).Skip(1).Any()
            Case Else
                Return False
        End Select
    End Function
...
#Region "下边栏"
    ...
    Private Sub BtnSelectED_Click(sender As MyIconTextButton, e As RouteEventArgs) Handles BtnSelectEnable.Click, BtnSelectDisable.Click
        EDMods(CompResourceListLoader.Output.Where(Function(m) SelectedMods.Contains(m.RawPath)).ToList(),
    ...
    Private Sub EDMods(ModList As IEnumerable(Of LocalCompFile), IsEnable As Boolean)
        ...
        For Each ModE In ModList
            ...
            Try
                ...
                End If
                File.Delete(NewPath)
                Microsoft.VisualBasic.FileSystem.Rename(ModEntity.Path, NewPath)
    ...
    Public Sub UpdateResource(ModList As IEnumerable(Of LocalCompFile))
        ...
        Try
            ...
            Dim PathMods As String = PageInstanceLeft.Instance.PathIndie & If(PageInstanceLeft.Instance.Info.HasLabyMod, "labymod-neo\fabric\" & PageInstanceLeft.Instance.Info.VanillaName & "\", "") & GetPathNameByCompType(CurrentCompType) & "\"
            ...
            For Each ModEntity In ModList
                ...
                Try
                    If ModEntity.IsFolder Then
                        ...
                        Else
                            FileSystem.DeleteDirectory(ModEntity.ActualPath, UIOption.AllDialogs, RecycleOption.SendToRecycleBin)
                        End If
                    Else
                        ' 删除文件
                        If IsShiftPressed Then
...
#Region "单个资源项"
    ...
    Public Sub Info_Click(sender As Object, e As EventArgs)
        Try
            ...
            Dim ModdedLabyMod = PageInstanceLeft.Instance.Info.HasLabyMod AndAlso PageInstanceLeft.Instance.Modable
            '加载失败信息
            ...
            If ModEntry.Comp IsNot Nothing Then
                ...
                FrmMain.PageChange(New FormMain.PageStackData With {.Page = FormMain.PageType.CompDetail,
                    .Additional = {ModEntry.Comp, New List(Of String), PageInstanceLeft.Instance.Info.VanillaName,
                        If(PageInstanceLeft.Instance.Info.HasForge, CompLoaderType.Forge,
                        If(PageInstanceLeft.Instance.Info.HasNeoForge, CompLoaderType.NeoForge,
                        If(PageInstanceLeft.Instance.Info.HasFabric OrElse ModdedLabyMod, CompLoaderType.Fabric, CompLoaderType.Any))), CurrentCompType}})
            Else
                ...
                If ModEntry.IsFolder Then
                    ...
                    If Directory.Exists(folderPath) Then
                        ...
                        Try
                            ...
                            Select Case CurrentCompType
                                Case CompType.Schematic
                                    fileCount = New DirectoryInfo(folderPath).EnumerateFiles("*", IO.SearchOption.AllDirectories).Where(Function(f) LocalCompFile.IsCompFile(f.FullName, CompType.Schematic)).Count()
                                Case CompType.Mod
                                    fileCount = New DirectoryInfo(folderPath).EnumerateFiles("*.jar", IO.SearchOption.AllDirectories).Count()
                                Case CompType.ResourcePack
                                    fileCount = New DirectoryInfo(folderPath).EnumerateFiles("*.zip", IO.SearchOption.AllDirectories).Count()
                                Case CompType.Shader
                                    fileCount = New DirectoryInfo(folderPath).EnumerateFiles("*.zip", IO.SearchOption.AllDirectories).Count()
                                Case Else
                                    fileCount = New DirectoryInfo(folderPath).EnumerateFiles("*", IO.SearchOption.AllDirectories).Count()
                            End Select
                        Catch ex As Exception
                            fileCount = 0
                        End Try
